        ...
        let config: Config = {
            let read_config = |default_config_path: PathBuf| -> String {
                ...
                if (!Path::new(&default_config_path).is_file()) || 
                    (Path::new(&default_config_path).is_file() && fs::read_to_string(&default_config_path).unwrap().is_empty()) {
                    // create .toml config file on default_config_path
                    if !Path::new(&default_config_path.parent().unwrap()).exists() {
                        if let Err(e) = std::fs::create_dir_all(default_config_path.parent().unwrap()) {
                            println!("{}", DisplayMsg::FailToCreateDir(String::from("config"), default_config_path.to_path_buf(), e.to_string()));
                            std::process::exit(1);
                        };
                    };
            ...
            toml::from_str(&read_config(default_config_path.clone()))
                .expect(&DisplayMsg::InvalidTOMLFormat(String::from("config"), default_config_path, String::new()).to_string())
        };
        ...
        if contents.trim() != "" {
            config_map = match serde_json::from_str(&contents){
                ...
                Err(_) => {
                    ...
                    println!("{}", DisplayMsg::InvalidTOMLFormat(String::from("config toml"), get_config_path(), String::new()));
                    std::process::exit(1);
                }
            };
        };
        config_map.insert("url".to_string(), Value::from(self.url.clone()));
        let toml_string = toml::to_string(&Value::Table(config_map))
                                .expect(&DisplayMsg::FailToTOMLEncode(String::from("config toml"), get_config_path(), String::new()).to_string());
                    
        match std::fs::write(get_config_path(), toml_string){
            Ok(_) => { println!("{}", DisplayMsg::ListRPCProvider(self.url.to_string()));  },
            Err(e) => {
                println!("{}", DisplayMsg::FailToWriteFile(String::from("config toml"), get_config_path(), e.to_string()));
                std::process::exit(1);
            }
        };
    }
...
pub fn get_home_dir() -> PathBuf {
    match std::env::var(PCHAIN_CLI_HOME_ENV_KEY){
        ...
        Err(_) => {
            println!("{}", DisplayMsg::PChainCliHomeNotSet(String::from(PCHAIN_CLI_HOME_ENV_KEY)));
            std::process::exit(1);
        }
    }
}
