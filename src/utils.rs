...
use rand::{distributions::Alphanumeric, Rng, thread_rng, RngCore, rngs::OsRng};
use argon2;
...
pub(crate) fn setup_password() -> Result<(), DisplayMsg>{
    ...
    let argon2_config = argon2::Config::default();
    let key = argon2::hash_raw(encoded_password.as_bytes(), &salt, &argon2_config).map_err(|e| DisplayMsg::FailToSetupPassword(e.to_string()))?;
...
pub(crate) fn encrypt(source: &[u8]) -> Result<Vec<u8>, DisplayMsg> {
    ...
    let encrypted = {
        ...
        let mut writer = encryptor.wrap_output(&mut encrypted).map_err(|e|  DisplayMsg::FailtoEncrypt(e.to_string()))?;
        writer.write_all(source).map_err(|e|  DisplayMsg::FailtoEncrypt(e.to_string()))?;
        writer.finish().map_err(|e|  DisplayMsg::FailtoEncrypt(e.to_string()))?;
...
pub(crate) fn decrypt(source: &[u8]) -> Result<Vec<u8>, DisplayMsg> {
    ...
    let decrypted = {
        let decryptor = match age::Decryptor::new(source).map_err(|e|  DisplayMsg::FailtoDecrypt(e.to_string()))? {
            age::Decryptor::Passphrase(d) => d,
            _ => unreachable!(),
        };
    
        let mut decrypted = vec![];
        let mut reader = decryptor.decrypt(&Secret::new(encoded_passphrase), None).map_err(|e|  DisplayMsg::FailtoDecrypt(e.to_string()))?;
...
pub fn write_file(path_to_file: PathBuf, content: &[u8]) -> Result<String, String> {
    ...
    Ok(dunce::canonicalize(path_to_file).unwrap().into_os_string().into_string().ok().unwrap())
}
